...
public class SpecificationReviewPhaseHandler extends AbstractPhaseHandler {
     ...
     * changed after that.
     */
    private static final Logger log = LoggerFactory.getLogger(SpecificationReviewPhaseHandler.class.getName());
     ...
     */
    public SpecificationReviewPhaseHandler(ManagerHelper managerHelper,
    ...
    private Review getSpecificationReview(Phase phase) throws PhaseHandlingException {
        ...
        if (reviews.length > 1) {
            log.error("Multiple specification reviews exist");
    ...
    private boolean checkSpecificationReview(Phase phase, String operator) throws PhaseHandlingException {
        ...
        if (null == submission) {
            log.error("The specification submission does not exist.");
        ...
        if (rejected) {
            ...
            try {
                ...
                log.error(new LogMessage(phase.getId(), null,
                        "Persistence layer error when updating submission status.").toString(), e);
    ...
    private boolean wasSpecificationReviewRejected(Phase phase) throws PhaseHandlingException {
        ...
        if (null == comment) {
            log.error("No comment with [Specification Review Comment] type.");
        ...
        if (Constants.COMMENT_VALUE_REJECTED.equalsIgnoreCase(value)) {
            ...
            log.error("Invalid comment[" + value + "],"
