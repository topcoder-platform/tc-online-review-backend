...
public class DatabaseProjectPaymentAdjustmentPersistence extends BaseProjectPaymentManagementPersistence implements
     ...
     *
     * @param projectPaymentAdjustment the project payment adjustment to create or update
     * @throws IllegalArgumentException                     if projectPaymentAdjustment is null
     * @throws ProjectPaymentAdjustmentValidationException  if projectPaymentAdjustment's project id is null, projectPaymentAdjustment's resource role id is
        ...
        try {
            ...
            try (Connection connection = Helper.getConnection(log, signature, getDbConnectionFactory(),
                ...
                try {
                    ...
                    // Update
                    if (Helper.executeUpdate(connection, false, SQL_UPDATE_PAYMENT_ADJUSTMENT,
                        ...
                        // Insert
                        Helper.executeUpdate(connection, false, SQL_INSERT_PAYMENT_ADJUSTMENT,
            ...
            throw LoggingWrapperUtility
                    .logException(log, signature, new ProjectPaymentManagementPersistenceException(
                            "Failed to save the project payment adjustment or close a resource.", e));
        }
    }
     ...
     *
     * @param projectId the project id of the project payment adjustment
    ...
    public List<ProjectPaymentAdjustment> retrieveByProjectId(long projectId)
        ...
        try {
            ...
            try (Connection connection = Helper.getConnection(log, signature, getDbConnectionFactory(), getConnectionName(), true);
                ...
                List<ProjectPaymentAdjustment> result = new ArrayList<>();
            ...
            throw LoggingWrapperUtility.logException(log, signature,
                    new ProjectPaymentManagementPersistenceException(
                            "Failed to retrieve the project payment adjustments or close a resource.", e));
        }
    }
     ...
     *
     * @param projectPaymentAdjustment the project payment adjustment.
     * @throws IllegalArgumentException                    if projectPaymentAdjustment is null
     * @throws ProjectPaymentAdjustmentValidationException if projectPaymentAdjustment's project id is null, projectPaymentAdjustment's resource role id is
     *                                                     null, projectPaymentAdjustment's fixed amount is negative, projectPaymentAdjustment's multiplier is
     *                                                     negative, if both fixed amount and multiplier are non-null at the same time.
    ...
    private static void validateProjectPaymentAdjustment(ProjectPaymentAdjustment projectPaymentAdjustment)
        ...
        if (isFixedAmountNotNull && (fixedAmount.compareTo(BigDecimal.ZERO) < 0)) {
