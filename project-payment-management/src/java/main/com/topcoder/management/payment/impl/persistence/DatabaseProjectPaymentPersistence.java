...
public class DatabaseProjectPaymentPersistence extends BaseProjectPaymentManagementPersistence implements
     ...
     *
     * @param projectPayment the project payment with updated data
     * @param operator       ID of the user doing the operation
     * @return the inserted project payment with generated id.
     * @throws IllegalArgumentException                       if projectPayment is null
     * @throws ProjectPaymentValidationException              if projectPayment's id is not null, projectPayment's type is null, projectPaymentType's
     ...
     * @throws ProjectPaymentManagementDataIntegrityException if project payment integrity is broken, for example if project payment is being persisted with
     *                                                        payment type that is not present in DB.
     * @throws ProjectPaymentManagementPersistenceException   if some other error occurred when accessing the persistence
     * @throws IllegalStateException                          if persistence was not configured properly (dbConnectionFactory is null)
     */
    public ProjectPayment create(ProjectPayment projectPayment, String operator) throws ProjectPaymentValidationException,
        ...
        try {
            ...
            try (Connection connection = Helper.getConnection(log, signature, getDbConnectionFactory(),
                    getConnectionName(), true)) {
                // Check project payment integrity
                checkProjectPaymentIntegrity(connection, projectPayment);

                // INSERT
                long projectPaymentId = Helper.executeUpdate(connection, true, SQL_INSERT_PAYMENT,
            ...
            throw LoggingWrapperUtility.logException(log, signature,
                    new ProjectPaymentManagementPersistenceException("Failed to create the project payment or close a resource.", e));
        }
    }
     ...
     * @param operator       ID of the user doing the operation
     * @throws IllegalArgumentException                       if projectPayment is null
     * @throws ProjectPaymentValidationException              if projectPayment's id is null or negative, projectPayment's type is null, projectPaymentType's
     *                                                        projectPaymentTypeId is null, projectPayment's resourceId is null, projectPayment's amount is null
     *                                                        or negative, projectPayment's createDate is null.
     * @throws ProjectPaymentNotFoundException                if project payment with ID equal to projectPayment.getProjectPaymentId() doesn't exist in
     *                                                        persistence
     * @throws ProjectPaymentManagementDataIntegrityException if project payment integrity is broken, for example if project payment is being persisted with
     *                                                        payment type that is not present in DB.
     * @throws ProjectPaymentManagementPersistenceException   if some other error occurred when accessing the persistence
    ...
    public void update(ProjectPayment projectPayment, String operator) throws ProjectPaymentValidationException,
        ...
        try {
            ...
            try (Connection connection = Helper.getConnection(log, signature, getDbConnectionFactory(),
                ...
                // UPDATE
                if (Helper.executeUpdate(connection, false, SQL_UPDATE_PAYMENT, projectPayment
            ...
            // Log exception
            throw LoggingWrapperUtility.logException(log, signature,
                    new ProjectPaymentManagementPersistenceException("Failed to update the project payment or close a resource.", e));
        }
    }
     ...
     * @throws ProjectPaymentManagementPersistenceException if some error occurred when accessing the persistence
     * @throws IllegalStateException                        if persistence was not configured properly (dbConnectionFactory is null)
     */
    public ProjectPayment retrieve(long projectPaymentId) throws ProjectPaymentManagementPersistenceException {
        ...
        try {
            try (Connection connection = Helper.getConnection(log, signature, getDbConnectionFactory(), getConnectionName(), true);
            ...
            // Log exception
            throw LoggingWrapperUtility.logException(log, signature,
                    new ProjectPaymentManagementPersistenceException("Failed to retrieve the project payment or close a resource.", e));
        }
    }
    ...
    public boolean delete(long projectPaymentId) throws ProjectPaymentManagementPersistenceException {
        ...
        try {
            try (Connection connection = Helper.getConnection(log, signature, getDbConnectionFactory(),
                ...
                // DELETE
                boolean result = (Helper.executeUpdate(connection, false, SQL_DELETE_PAYMENT,
                        projectPaymentId) != 0);
     ...
     * @throws ProjectPaymentManagementConfigurationException if some error occurred when initializing an instance using the given configuration (is not thrown
     ...
     */
    private void checkProjectPaymentIntegrity(Connection connection,
                                              ProjectPayment projectPayment) throws ProjectPaymentManagementDataIntegrityException, SQLException {
        checkRecord("projectPaymentTypeId", connection, "project_payment_type_lu",
                "project_payment_type_id", projectPayment.getProjectPaymentType().getProjectPaymentTypeId());
        checkRecord("resourceId", connection, "resource", "resource_id", projectPayment
                .getResourceId());

        Long submissionId = projectPayment.getSubmissionId();
        if (submissionId != null) {
            checkRecord("submissionId", connection, "submission", "submission_id", submissionId);
        }
    }
     ...
     * @throws SQLException                                   if any other error occurs
     */
    private static void checkRecord(String fieldName, Connection connection, String table,
     ...
     * @throws IllegalArgumentException          if projectPayment is null
     * @throws ProjectPaymentValidationException if projectPayment's id is not null (when create is true) or projectPayment's id is null or negative
