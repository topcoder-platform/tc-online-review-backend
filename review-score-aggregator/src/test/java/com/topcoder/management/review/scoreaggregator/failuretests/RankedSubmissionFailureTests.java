...
public class RankedSubmissionFailureTests extends TestCase {
    ...
    public void testRankedSubmissionNegativeRank() {
        try {
            new RankedSubmission(new AggregatedSubmission(new Submission(1L, new double[] {94.0f}), 94.0f), -1);
            fail("IllegalArgumentException should be thrown.");
        } catch (IllegalArgumentException e) {
            // expected
        }
    ...
    public void testRankedSubmissionZeroRank() {
        try {
            new RankedSubmission(new AggregatedSubmission(new Submission(1L, new double[] {94.0f}), 94.0f), 0);
            fail("IllegalArgumentException should be thrown.");
        } catch (IllegalArgumentException e) {
            // expected
        }
