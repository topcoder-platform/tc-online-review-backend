...
public class AggregationPhaseHandler extends AbstractPhaseHandler {
     ...
     */
    private static final Logger log = LoggerFactory.getLogger(AggregationPhaseHandler.class.getName());
     ...
     */
    public AggregationPhaseHandler(ManagerHelper managerHelper,
    ...
    private void createAggregationWorksheet(Phase phase, String operator)
        ...
        try {
            ...
            if (submissions == null || submissions.length == 0) {
                log.error("No winner for project with id" + phase.getProject().getId());
                throw new PhaseHandlingException(
            ...
            log.debug(new LogMessage(phase.getId(), operator, "create aggregate worksheet.").toString());
