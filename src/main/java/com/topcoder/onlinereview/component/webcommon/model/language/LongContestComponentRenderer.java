...
public class LongContestComponentRenderer extends BaseRenderer implements ElementRenderer {
    ...
    public String toHTML(Language language) throws Exception {
        ...
        buf.append("</td>");
        buf.append("</tr>");
        String[] exposedMethodNames = this.problemComponent.getAllExposedMethodNames();
        DataType[][] exposedAllParamTypes = this.problemComponent.getAllExposedParamTypes();
        String[][] exposedAllParamNames = this.problemComponent.getAllExposedParamNames();
        DataType[] exposedReturnTypes = this.problemComponent.getAllExposedReturnTypes();
        int j;
        if (exposedMethodNames.length != 0) {
            ...
            buf.append(">");
            appendTag(buf, this.SECTION_HEADER, "Available Libraries");
            buf.append("</td>");
            buf.append("</tr>");
            buf.append("<tr>");
            buf.append("<td");
            ...
            buf.append(">");
            buf.append(this.problemComponent.getExposedClassName());
            buf.append("</td>");
            buf.append("</tr>");
            ...
            for(int i = 0; i < exposedMethodNames.length; ++i) {
                ...
                buf.append(">");
                buf.append(encodeHTML(language.exampleExposedCall(this.problemComponent.getExposedClassName(), exposedMethodNames[i], exposedAllParamNames[i])));
                buf.append("</td>");
                buf.append("</tr>");
                buf.append("<tr><td colspan=\"2\"");
                if (this.getTdClass() != null) {
        ...
        if (constraints != null && constraints.length > 0) {
            ...
            for(j = 0; j < constraints.length; ++j) {
                ...
                buf.append(">");
                buf.append(super.getRenderer(constraints[j]).toHTML(language));
                buf.append("</td>");
                buf.append("</tr>");
            }
        }
