...
public class DataRetriever {
   ...
   * @param entityManager
   */
  public DataRetriever(EntityManager entityManager) {
  ...
  private String runDefaultInputQuery(String defaultQueryId, Map inputs) throws Exception {
    ...
    var rs = executeSql(entityManager, "SELECT text FROM query WHERE query_id=" + specialQueryId);
    if (rs.isEmpty()) {
      throw new Exception("Query text for query ID " + specialQueryId + " missing from DB");
    }
    var specialQuery = getString(rs.get(0), "text");
    ...
    var rs2 = executeSql(entityManager, specialQuery);
    if (rs2.isEmpty()) {
  ...
  public Map<String, List<Map<String, Object>>> executeCommand(Map inputMap) throws Exception {
    ...
    try {
      ...
      var rs = executeSqlWithParam(entityManager, query.toString(), newArrayList(commandDesc));
      rowcount = 0;
      qid = new ArrayList();
    ...
    try {
      ...
      var rs1 = executeSqlWithParam(entityManager, query.toString(), newArrayList(commandDesc));
    ...
    try {
      ...
      for (i = 0; i < queryIdList.length; i++) {
        try {
          ...
          //          }
          var rs = executeSql(entityManager, queryText);
