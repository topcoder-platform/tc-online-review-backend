...
/**
 * This is the default implementation of the ScorecardValidator interface to provide scorecard
 * validation functions.
 *
 * <p>Thread safety: This class is immutable and thread safe.<br>
 *
 * @author tuenm, zhuzeyuan
 * @version 1.0.1
 */
@Component
public class ScorecardValidator {

  /** The precision error to be used while comparing two floating numbers. */
  private static final float EPS = 1e-9f;

  /**
   * The floating number for the total weight of a whole scorecard/group/section. This number should
   * be 100.
   */
   ...
   * @throws ValidationException if the float is out of range [0,100].
   */
  private void assertFloatBetween0And100(float number, String name) throws ValidationException {
    if (number < -EPS || number > ONE_HUNDRED + EPS) {
      throw new ValidationException(
          "The floating number " + name + " must be between 0 and 100, inclusive.");
    }
  }
   ...
   * @throws ValidationException if the string is null, empty or longer than allowance.
   */
  private void checkNameString(String s, String name) throws ValidationException {
    ...
    if (s.length() > NAME_MAXLENGTH) {
      throw new ValidationException(
          "Length of the string " + name + " must be smaller than or equal to " + NAME_MAXLENGTH);
    }
  }
   ...
   * @throws ValidationException if the version string is illegal.
   */
  private void checkVersion(String version) throws ValidationException {
    ...
    if (version.length() > VERSION_MAXLENGTH) {
      throw new ValidationException(
          "Length of the version string must be smaller than or equal to 16.");
    }
    for (int i = 0; i < version.length(); i++) {
      if (!Character.isDigit(version.charAt(i)) && version.charAt(i) != '.') {
        throw new ValidationException(
            "Every single char in version string must be a digit or a dot.");
      }
    }
    if (version.length() == 0) {
   ...
   * ValidationException on the first item that is not follow the a rule. For the set of rules, see
   * class documentation of <code>validateScorecard</code>. This class is used as a helper method
   * for <code>validateScorecard</code>.<br>
   *
  ...
  private void validateGroup(Group group) throws ValidationException {
    ...
    // While getting weight from the group/section/question, IllegalStateException might be thrown
    // while the value
    // is not set properly.
    try {
   ...
   * throw ValidationException on the first item that is not follow the a rule.For the set of rules,
   * see class documentation of this class.<br>
   *
   * @param scorecard The scorecard to validate.
   * @throws IllegalArgumentException if the given scorecard is null.
   * @throws ValidationException if validation fails.
   */
  public void validateScorecard(Scorecard scorecard) throws ValidationException {
    ...
    try {
      ...
      // The minScore cannot be smaller than 0, while the maxScore cannot be smaller than or equal
      // to the
      // minScore.
