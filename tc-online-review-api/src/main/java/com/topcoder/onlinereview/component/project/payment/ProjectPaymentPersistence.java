...
public class ProjectPaymentPersistence {
  ...
  @Value("${payment.persistence.entity-manager-name}")
  ...
  public ProjectPayment retrieve(long projectPaymentId)
    ...
    try {
      ...
      var resultSet =
          executeSqlWithParam(entityManager, SQL_QUERY_PAYMENT, newArrayList(projectPaymentId));
      ProjectPayment result = null;
      if (!resultSet.isEmpty()) {
        var f = resultSet.get(0);
  ...
  public boolean delete(long projectPaymentId) throws ProjectPaymentManagementPersistenceException {
    ...
    try {
      executeSqlWithParam(entityManager, SQL_DELETE_PAYMENT, newArrayList(projectPaymentId));
      // Log Exit
  ...
  private void checkRecord(String fieldName, String table, String key, Object value)
    ...
    var result = executeSqlWithParam(entityManager, sql, newArrayList(value));
