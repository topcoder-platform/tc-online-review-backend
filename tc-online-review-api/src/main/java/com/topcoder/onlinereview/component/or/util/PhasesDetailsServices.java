...
public class PhasesDetailsServices {
    ...
    private static boolean hasPermission(String roles, String... requires) {
        return true;
    }

    private static boolean hasUserRole(String roles, String... requireRoles) {
        return true;
    }
     ...
     */
    public static PhasesDetails getPhasesDetails(UserRetrieval userRetrieval,LookupHelper lookupHelper, ResourceManager resourceManager, ReviewManager reviewManager, UploadManager uploadManager, String roles,
                                                 HttpServletRequest request, MessageSource textProvider, Project project,
        ...
        Submission[] mostRecentContestSubmissions = ActionsHelper.getProjectSubmissions(lookupHelper, uploadManager,
                project.getId(), ORConstants.CONTEST_SUBMISSION_TYPE_NAME, null, false);
        ...
        for (int phaseIdx = 0; phaseIdx < phases.length; ++phaseIdx) {
            ...
            if (phaseGroup == null || !ConfigHelper.isPhaseGroupContainsPhase(phaseGroupIdx, phaseName)
                ...
                if (!appFuncName.equals(ORConstants.VIEW_REGISTRANTS_APP_FUNC)
                        || hasPermission(roles, ORConstants.VIEW_REGISTRATIONS_PERM_NAME)) {
                    ...
                    if (appFuncName.equals(ORConstants.VIEW_REVIEWS_APP_FUNC) && ActionsHelper.isStudioProject(project)) {
                        phaseGroup.setName(
                                getMessageText(textProvider, "ProjectPhaseGroup.Review", new String[] { groupIndexStr }));
                    } else {
                        phaseGroup.setName(getMessageText(textProvider,ConfigHelper.getPhaseGroupNameKey(phaseGroupIdx),
                                new String[] { groupIndexStr }));
                    }
                    phaseGroup.setTableName(getMessageText(textProvider,ConfigHelper.getPhaseGroupTableNameKey(phaseGroupIdx),
                            new String[] { groupIndexStr }));
            ...
            submitters = getSubmitters(roles, allProjectResources, isAfterAppealsResponse, isStudioScreening,
                    submitters);
            ...
            if (phaseGroup.getAppFunc().equals(ORConstants.VIEW_REGISTRANTS_APP_FUNC)) {
                ...
                serviceSubmissionsAppFunc(uploadManager, reviewManager, lookupHelper, roles, request, phaseGroup, project, phases, phaseIdx, allProjectResources,
                ...
                serviceReviewsAppFunc(uploadManager, lookupHelper, roles, request, phaseGroup, project, phase, nextPhase, allProjectResources, submitters,
                ...
                serviceAggregationAppFunc(uploadManager, reviewManager, lookupHelper, roles, phaseGroup, project, phase, allProjectResources,
                        mostRecentContestSubmissions, isAfterAppealsResponse);
            } else if (phaseGroup.getAppFunc().equalsIgnoreCase(ORConstants.FINAL_FIX_APP_FUNC)) {
                serviceFinalFixAppFunc(uploadManager, reviewManager, lookupHelper, roles, phaseGroup, project, phase, allProjectResources,
                        mostRecentContestSubmissions, isAfterAppealsResponse);
            } else if (phaseGroup.getAppFunc().equalsIgnoreCase(ORConstants.APPROVAL_APP_FUNC)) {
                serviceApprovalAppFunc(reviewManager, uploadManager, lookupHelper, roles, phaseGroup, project, phase, allProjectResources, isAfterAppealsResponse,
                ...
                serviceSpecReviewAppFunc(lookupHelper, reviewManager, resourceManager, phaseGroup, project, phase, allProjectResources);
            } else if (phaseGroup.getAppFunc().equalsIgnoreCase(ORConstants.CHECKPOINT_APP_FUNC)) {
                serviceCheckpointAppFunc(lookupHelper, reviewManager, uploadManager, roles, request, phaseGroup, project, phase, allProjectResources, phases, phaseIdx,
                        submitters);
            } else if (phaseGroup.getAppFunc().equalsIgnoreCase(ORConstants.ITERATIVEREVIEW_APP_FUNC)) {
                serviceIterativeReviewsAppFunc(uploadManager, lookupHelper, reviewManager, roles, request, phaseGroup, project, phase, allProjectResources, submitters,
                        mostRecentContestSubmissions);
            }
        }
     ...
     */
    private static void serviceCheckpointAppFunc(LookupHelper lookupHelper, ReviewManager reviewManager,UploadManager uploadManager,String roles, HttpServletRequest request, PhaseGroup phaseGroup, Project project,
            Phase phase, Resource[] allProjectResources, Phase[] phases, int phaseIdx, Resource[] submitters)
            throws BaseException {
        Phase checkpointReviewPhase = ActionsHelper.getPhase(phases, false, ORConstants.CHECKPOINT_REVIEW_PHASE_NAME);
        if (checkpointReviewPhase != null) {
            phaseGroup.setCheckpointReviewFinished(checkpointReviewPhase.getPhaseStatus().getId() == 3);
        }

        Phase reviewPhase = ActionsHelper.findPhaseByTypeName(phases, ORConstants.APPEALS_RESPONSE_PHASE_NAME);
        if (reviewPhase == null) {
        ...
        boolean mayViewMostRecentAfterReview = hasPermission(roles,
                ORConstants.VIEW_RECENT_CHECKPOINT_SUBMISSIONS_AFTER_REVIEW_PERM_NAME);
        ...
        if (phaseName.equalsIgnoreCase(ORConstants.CHECKPOINT_SUBMISSION_PHASE_NAME)) {
            ...
            if (mayViewMostRecentAfterReview && isReviewFinished
                    ...
                    || (hasPermission(roles,
                    ORConstants.VIEW_RECENT_CHECKPOINT_SUBMISSIONS_PERM_NAME)
                            && !hasUserRole(roles, ORConstants.CHECKPOINT_REVIEWER_ROLE_NAME)
                            && !hasUserRole(roles, ORConstants.REVIEWER_ROLE_NAMES))
                    || (hasPermission(roles,
                    ORConstants.VIEW_RECENT_CHECKPOINT_SUBMISSIONS_PERM_NAME)
                            && hasUserRole(roles, ORConstants.CHECKPOINT_REVIEWER_ROLE_NAME)
                            && ActionsHelper.isInOrAfterPhase(phases, phaseIdx, ORConstants.CHECKPOINT_REVIEW_PHASE_NAME))
                    || (hasPermission(roles,
                    ORConstants.VIEW_RECENT_CHECKPOINT_SUBMISSIONS_PERM_NAME)
                            && hasUserRole(roles, ORConstants.REVIEWER_ROLE_NAMES)
                            && ActionsHelper.isInOrAfterPhase(phases, phaseIdx, ORConstants.REVIEW_PHASE_NAME))
                    || (hasPermission(roles,
                    ORConstants.VIEW_SCREENER_CHECKPOINT_SUBMISSION_PERM_NAME)
                            && ActionsHelper.isInOrAfterPhase(phases, phaseIdx,
                    ORConstants.CHECKPOINT_SCREENING_PHASE_NAME))) {
                submissions = ActionsHelper.getProjectSubmissions(lookupHelper, uploadManager, project.getId(),
                        ORConstants.CHECKPOINT_SUBMISSION_TYPE_NAME, null, false);
            }

            if (submissions == null && hasPermission(roles,
                    ORConstants.VIEW_MY_CHECKPOINT_SUBMISSIONS_PERM_NAME)) {
                ...
                submissions = ActionsHelper.getResourceSubmissions(uploadManager,lookupHelper,myResource.getId(),
                        ORConstants.CHECKPOINT_SUBMISSION_TYPE_NAME, null, false);
            }
            ...
            Comparators.CheckpointSubmissionComparator comparator = new Comparators.CheckpointSubmissionComparator();

            comparator.assignSubmitters(submitters);
            Arrays.sort(submissions, comparator);

            phaseGroup.setPastCheckpointSubmissions(getPreviousUploadsForSubmissions(lookupHelper, uploadManager, roles, project, submissions,
                    ORConstants.VIEW_ALL_CHECKPOINT_SUBMISSIONS_PERM_NAME));
        ...
        if (phaseName.equalsIgnoreCase(ORConstants.CHECKPOINT_SCREENING_PHASE_NAME)
            ...
            phaseGroup.setCheckpointScreeningReviews(ActionsHelper.searchReviews(reviewManager, phase.getId(), null, false));
        }
        ...
        if (phaseName.equalsIgnoreCase(ORConstants.CHECKPOINT_REVIEW_PHASE_NAME)
            ...
            phaseGroup.setCheckpointReviews(ActionsHelper.searchReviews(reviewManager, phase.getId(), null, false));
        }
    }
     ...
     */
    private static void serviceRegistrantsAppFunc(UserRetrieval userRetrieval, PhaseGroup phaseGroup,
                                                  Resource[] submitters, ExternalUser[] allProjectExternalUsers) throws RetrievalException {
     ...
     * @throws BaseException if an unexpected error occurs.
     */
    private static Upload[][] getPreviousUploadsForSubmissions(LookupHelper lookupHelper, UploadManager uploadManager, String roles, Project project,
                                                               Submission[] submissions, String viewAllSubmissionsPermission) throws BaseException {
        Upload[][] pastSubmissions = null;
        if (submissions.length > 0 && hasPermission(roles, viewAllSubmissionsPermission)) {

            // Find all deleted submissions for specified project
            Submission[] allDeletedSubmissions = ActionsHelper.getProjectSubmissions(lookupHelper, uploadManager, project.getId(), null, "Deleted",
                    true);
     ...
     * @throws BaseException the base exception
     */
    private static void serviceSubmissionsAppFunc(UploadManager uploadManager,ReviewManager reviewManager, LookupHelper lookupHelper, String roles, HttpServletRequest request, PhaseGroup phaseGroup, Project project,
        ...
        if (phaseName.equalsIgnoreCase(ORConstants.SUBMISSION_PHASE_NAME)) {
            ...
            if (hasPermission(roles, ORConstants.VIEW_ALL_SUBM_PERM_NAME)) {
                submissions = mostRecentContestSubmissions;
            }

            boolean mayViewMostRecentAfterAppealsResponse = hasPermission(roles,
                    ORConstants.VIEW_RECENT_SUBM_AAR_PERM_NAME);
            ...
            if (submissions == null
                    && hasPermission(roles, ORConstants.VIEW_RECENT_SUBM_PERM_NAME)
                    && !hasUserRole(roles, ORConstants.REVIEWER_ROLE_NAMES)) {
                submissions = mostRecentContestSubmissions;
            }
            if (submissions == null
            ...
            if (submissions == null
                    && hasPermission(roles, ORConstants.VIEW_SCREENER_SUBM_PERM_NAME)
                    && ActionsHelper.isInOrAfterPhase(phases, phaseIdx, ORConstants.SCREENING_PHASE_NAME)) {
                submissions = mostRecentContestSubmissions;
            }
            if (submissions == null
                    && hasPermission(roles, ORConstants.VIEW_MY_SUBM_PERM_NAME)) {
                // Get "my" (submitter's) resource
                Resource myResource = null;
                // TODO myResources
                Resource[] myResources = ActionsHelper.getMyResourcesForPhase(request, null);
            ...
            if (submissions == null && hasPermission(roles,
                    ORConstants.VIEW_CURRENT_ITERATIVE_REVIEW_SUBMISSION)) {
                submissions = mostRecentContestSubmissions;
            ...
            phaseGroup.setPastSubmissions(
                    getPreviousUploadsForSubmissions(lookupHelper, uploadManager, roles, project, submissions, ORConstants.VIEW_ALL_SUBM_PERM_NAME));

            phaseGroup.setSubmissions(submissions);
        }

        if (phaseName.equalsIgnoreCase(ORConstants.SCREENING_PHASE_NAME) && phaseGroup.getSubmissions() != null) {
            Submission[] submissions = phaseGroup.getSubmissions();

            if (hasPermission(roles, ORConstants.VIEW_SCREENER_SUBM_PERM_NAME)
            ...
            phaseGroup.setScreenings(ActionsHelper.searchReviews(reviewManager, phases[phaseIdx].getId(), null, false));
        }
    }
     ...
     * @throws BaseException the base exception
     */
    private static void serviceReviewsAppFunc(UploadManager uploadManager, LookupHelper lookupHelper, String roles, HttpServletRequest request, PhaseGroup phaseGroup, Project project,
        ...
        if (phaseName.equalsIgnoreCase(ORConstants.REVIEW_PHASE_NAME)) {
            ...
            boolean allowedToSeeReviewLink = hasUserRole(roles,
                    new String[] { ORConstants.MANAGER_ROLE_NAME, ORConstants.GLOBAL_MANAGER_ROLE_NAME,
                            ORConstants.COCKPIT_PROJECT_USER_ROLE_NAME, ORConstants.REVIEWER_ROLE_NAME,
                            ORConstants.ACCURACY_REVIEWER_ROLE_NAME, ORConstants.FAILURE_REVIEWER_ROLE_NAME,
                            ORConstants.STRESS_REVIEWER_ROLE_NAME, ORConstants.CLIENT_MANAGER_ROLE_NAME,
                            ORConstants.COPILOT_ROLE_NAME, ORConstants.OBSERVER_ROLE_NAME });
            // Determine if the Review phase is closed
            boolean isReviewClosed = phase.getPhaseStatus().getName().equalsIgnoreCase(ORConstants.CLOSED_PH_STATUS_NAME);
            ...
            boolean isAppealsOpen = (nextPhase != null
                    ...
                    && nextPhase.getPhaseStatus().getName().equalsIgnoreCase(ORConstants.OPEN_PH_STATUS_NAME));
            ...
            if (!allowedToSeeReviewLink) {
                ...
                if (isReviewClosed || canViewReviewsDuringReview || (isAppealsOpen
                        && hasPermission(roles, ORConstants.PERFORM_APPEAL_PERM_NAME))) {
            ...
            if (!isAfterAppealsResponse
                    && hasPermission(roles, ORConstants.VIEW_REVIEWER_REVIEWS_PERM_NAME)
                    && hasUserRole(roles, ORConstants.REVIEWER_ROLE_NAMES)) {
            ...
            boolean canDownloadTestCases = (isReviewClosed
                    && hasPermission(roles, ORConstants.DOWNLOAD_TEST_CASES_PERM_NAME))
                    || (!isReviewClosed && hasPermission(roles,
                    ORConstants.DOWNLOAD_TC_DUR_REVIEW_PERM_NAME))
        ...
        if (phaseName.equalsIgnoreCase(ORConstants.APPEALS_PHASE_NAME) && phaseGroup.getReviews() != null) {
     ...
     * @throws BaseException the base exception
     */
    private static void serviceAggregationAppFunc(UploadManager uploadManager, ReviewManager reviewManager, LookupHelper lookupHelper, String roles, PhaseGroup phaseGroup, Project project,
        ...
        retrieveSubmissions(uploadManager, lookupHelper, roles, null,  phaseGroup, project, mostRecentContestSubmissions, isAfterAppealsResponse);
        String phaseName = phase.getPhaseType().getName();

        if (phaseName.equalsIgnoreCase(ORConstants.AGGREGATION_PHASE_NAME) && phaseGroup.getSubmitters() != null) {
            ...
            Review[] reviews = ActionsHelper.searchReviews(reviewManager, phase.getId(), aggregator[0].getId(), false);
            if (reviews.length != 0) {
     ...
     * @throws BaseException the base exception
     */
    private static void serviceFinalFixAppFunc(UploadManager uploadManager, ReviewManager reviewManager, LookupHelper lookupHelper, String roles, PhaseGroup phaseGroup, Project project,
            Phase phase, Resource[] allProjectResources, Submission[] mostRecentContestSubmissions,
            boolean isAfterAppealsResponse) throws BaseException {
        // TODO myResources
        retrieveSubmissions(uploadManager, lookupHelper, roles, null, phaseGroup, project, mostRecentContestSubmissions, isAfterAppealsResponse);
        ...
        if (phaseName.equalsIgnoreCase(ORConstants.FINAL_FIX_PHASE_NAME)) {
            Upload[] uploads = ActionsHelper.getPhaseUploads(uploadManager, lookupHelper, phase.getId(), "Final Fix");
        ...
        if (phaseName.equalsIgnoreCase(ORConstants.FINAL_REVIEW_PHASE_NAME) && phaseGroup.getSubmitters() != null) {
            Resource[] reviewers = ActionsHelper.getResourcesForPhase(allProjectResources, phase);
            ...
            Review[] reviews = ActionsHelper.searchReviews(reviewManager, phase.getId(), reviewers[0].getId(), false);
            if (reviews.length != 0) {
     ...
     * @throws BaseException if an unexpected error occurs.
     */
    private static void serviceSpecReviewAppFunc(LookupHelper lookupHelper, ReviewManager reviewManager,
                                                 ResourceManager resourceManager,
                                                 PhaseGroup phaseGroup, Project project, Phase phase,
        ...
        if (phaseName.equalsIgnoreCase(ORConstants.SPECIFICATION_SUBMISSION_PHASE_NAME)) {
            SubmissionType specSubmissionType = lookupHelper
                    .getSubmissionType(ORConstants.SPECIFICATION_SUBMISSION_TYPE_NAME);
     ...
     * @throws BaseException if an unexpected error occurs.
     */
    private static void serviceApprovalAppFunc(ReviewManager reviewManager, UploadManager uploadManager, LookupHelper lookupHelper, String roles, PhaseGroup phaseGroup, Project project,
        ...
        retrieveSubmissions(uploadManager, lookupHelper, roles, allProjectResources, phaseGroup, project, mostRecentContestSubmissions, isAfterAppealsResponse);
        ...
        phaseGroup.setApproval(ActionsHelper.searchReviews(reviewManager, thisPhase.getId(), null, false));
     ...
     * @throws BaseException if an unexpected error occurs.
     */
    private static void servicePostMortemAppFunc(ReviewManager reviewManager, PhaseGroup phaseGroup, Project project, Phase thisPhase,
            Resource[] allProjectResources) throws BaseException {
        ...
        phaseGroup.setPostMortemReviews(ActionsHelper.searchReviews(reviewManager, thisPhase.getId(), null, false));
     ...
     * @throws BaseException if an unexpected error occurs.
     */
    private static void serviceIterativeReviewsAppFunc(UploadManager uploadManager, LookupHelper lookupHelper, ReviewManager revMgr, String roles,
                                                       HttpServletRequest request, PhaseGroup phaseGroup,
        ...
        if (phaseName.equalsIgnoreCase(ORConstants.ITERATIVE_REVIEW_PHASE_NAME)) {
            ...
            if (hasPermission(roles, ORConstants.PERFORM_ITERATIVE_REVIEW_PERM_NAME)) {
                phaseGroup.setIterativeReviewSubmission(associatedSubmission);
            }
            ...
            if (phaseGroup.getIterativeReviewSubmission() == null
                ...
                Submission[] mySubmissions = ActionsHelper.getResourceSubmissions(uploadManager, lookupHelper, myResource.getId(),
                        ORConstants.CONTEST_SUBMISSION_TYPE_NAME, null, false);
            ...
            boolean allowedToSeeReviewLink = hasUserRole(roles,
                    new String[] { ORConstants.MANAGER_ROLE_NAME, ORConstants.GLOBAL_MANAGER_ROLE_NAME,
                            ORConstants.COCKPIT_PROJECT_USER_ROLE_NAME, ORConstants.ITERATIVE_REVIEWER_ROLE_NAME,
                            ORConstants.CLIENT_MANAGER_ROLE_NAME, ORConstants.COPILOT_ROLE_NAME,
            ...
            if (phase.getPhaseStatus().getName().equalsIgnoreCase("Open")
                    && hasPermission(roles,
                    ORConstants.VIEW_ITERATIVE_REVIEWER_REVIEWS_PERM_NAME)
                    && hasUserRole(roles, ORConstants.ITERATIVE_REVIEWER_ROLE_NAME)) {
                // Get "my" (reviewer's) resource
                reviewers = new Resource[1];
//                Resource[] myResources
                reviewers[0] = ActionsHelper.getMyResourceForRole(myResources, ORConstants.ITERATIVE_REVIEWER_ROLE_NAME);
            }
     ...
     * @throws BaseException the base exception
     */
    private static void retrieveSubmissions(UploadManager uploadManager, LookupHelper lookupHelper, String roles, Resource[] myResources, PhaseGroup phaseGroup, Project project,
            Submission[] mostRecentContestSubmissions, boolean isAfterAppealsResponse) throws BaseException {
        ...
        if ((mayViewMostRecentAfterAppealsResponse && isAfterAppealsResponse)
                || hasPermission(roles, ORConstants.VIEW_ALL_SUBM_PERM_NAME)
                || hasPermission(roles, ORConstants.VIEW_RECENT_SUBM_PERM_NAME)
                || hasPermission(roles, ORConstants.VIEW_WINNING_SUBM_PERM_NAME)) {
            submissions = mostRecentContestSubmissions;
        }
        if (submissions == null && hasPermission(roles, ORConstants.VIEW_MY_SUBM_PERM_NAME)) {
            ...
            submissions = ActionsHelper.getResourceSubmissions(uploadManager, lookupHelper, myResource.getId(),
                    ORConstants.CONTEST_SUBMISSION_TYPE_NAME, ORConstants.ACTIVE_SUBMISSION_STATUS_NAME, false);
        }
        if (submissions != null) {
            phaseGroup.setSubmissions(submissions);
        }
     ...
     */
    private static Resource[] getSubmitters(String roles, Resource[] allProjectResources,
        ...
        final boolean canSeeSubmitters = (isAfterAppealsResponse
                || hasPermission(roles, ORConstants.VIEW_ALL_SUBM_PERM_NAME)
                || (isStudioScreening && (hasUserRole(roles, ORConstants.SCREENER_ROLE_NAME)
