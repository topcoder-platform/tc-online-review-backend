...
public class UserRetrieval {
  ...
  @Value("${external.persistence.entity-manager-name}")
  ...
  private Map<Long, ExternalUser> selectEmail(
    ...
    var result = executeSqlWithParam(entityManager, emailQuery + queryAndClause, queryParameter);
    for (var m : result) {
  ...
  private Map selectRating(
    ...
    var result = executeSqlWithParam(entityManager, ratingsQuery + queryAndClause, queryParameter);
    result.forEach(
        m -> {
          Optional.ofNullable(userMap.get(getLong(m, "id")))
              .ifPresent(
