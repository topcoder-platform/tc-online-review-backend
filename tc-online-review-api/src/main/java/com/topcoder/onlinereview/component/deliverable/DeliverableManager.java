...
public class DeliverableManager {
     ...
     */
    public DeliverableManager(DeliverablePersistence persistence,
        ...
        DeliverableHelper.checkObjectNotNull(persistence, "persistence", log);
        DeliverableHelper.checkObjectNotNull(deliverableCheckers, "deliverableCheckers", log);
        DeliverableHelper.checkObjectNotNullFullDesp(deliverableSearchBundle,
                "deliverableSearchBundle is null, or searchBundleManager doesn't contain"
                        + " SearchBundle of required names DELIVERABLE_SEARCH_BUNDLE_NAME", log);
        DeliverableHelper.checkObjectNotNullFullDesp(submissionSearchBundle,
                "submissionSearchBundle is null, or searchBundleManager doesn't contain"
                        + " SearchBundle of required names DELIVERABLE_WITH_SUBMISSIONS_SEARCH_BUNDLE_NAME", log);
        ...
        log.debug("Instantiate PersistenceDeliverableManager with DeliverablePersistence, deliverableCheckers map,"
    ...
    private static long[][] checkAndGetCustomResultSetValidDeliverable(Object obj, int maxColumn)
        ...
        if (!(obj instanceof CustomResultSet)) {
            log.error("the object get from SearchBundle is not a CustomResultSet");
        ...
        log.debug("CustomResultSet Records: " + customerResult.getRecordCount());
        ...
        try {
            ...
            log.error("the CustomResultSet get from SearchBundle"
            ...
            log.error("error occurs when fetching the current row. \n"
                    + LogMessage.getExceptionStackTrace(e));
