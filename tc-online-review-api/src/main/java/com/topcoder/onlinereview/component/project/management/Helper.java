...
class Helper {
     ...
     */
    static int doDMLQuery(EntityManager entityManager, String queryString, Object[] queryArgs) throws PersistenceException {
        Helper.assertObjectNotNull(entityManager, "connection");
     ...
     */
    static boolean checkEntityExists(String tableName, String columnName, long id, EntityManager entityManager)
        ...
        Helper.assertObjectNotNull(entityManager, "entityManager");

        var result = executeSql(entityManager, String.format("SELECT COUNT(*) as count FROM %s WHERE %s = %d", tableName, columnName, id));
