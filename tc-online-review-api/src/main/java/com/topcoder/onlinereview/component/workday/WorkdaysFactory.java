...
public class WorkdaysFactory {
  ...
  private Optional<String> localeLanguage;

  @Value("${workday.locale.country:#{null}}")
  private Optional<String> localeCountry;

  @Value("${workday.locale.variant:#{null}}")
  private Optional<String> localeVariant;

  @Value("${workday.date_style:#{null}}")
  private Optional<String> dateStyle;

  @Value("${workday.non_workdays:#{null}}")
  private Optional<String> nonWorkdays;
  ...
  public void postRun() {
    ...
    if (localeLanguage.isPresent() && localeCountry.isPresent() && localeVariant.isPresent()) {
      this.locale = new Locale(localeLanguage.get(), localeCountry.get(), localeVariant.get());
    } else if (localeLanguage.isPresent() && localeCountry.isPresent()) {
      // only language and country are specified
      this.locale = new Locale(localeLanguage.get(), localeCountry.get());
    }
    dateStyle.ifPresentOrElse(
        style -> {
        ...
        () -> dateFormat = DateFormat.getDateInstance(DateFormat.DEFAULT, this.locale));
    // load all non workdays
    nonWorkdays.ifPresent(
        nws -> {
          Stream.of(nws.split(";"))
              .forEach(
                  d -> {
