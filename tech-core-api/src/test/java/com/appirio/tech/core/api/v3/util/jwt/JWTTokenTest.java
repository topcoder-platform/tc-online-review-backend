 ...
 * - enhance the test parse  method to test the scope field
 * </p>
 * 
 * <p>
...
public class JWTTokenTest {
	...
	private static List<String> validIssuers = Arrays.asList("https://topcoder-newauth.auth0.com/");
	...
	public void testWalkThrough() throws Exception {
		...
		// decode token
		JWTToken jwtDec = new JWTToken(token, secret,validIssuers);
	...
	public void testVerifyToken_ThrowsInvalidTokenExceptionWhenSecretIsDifferent() throws Exception {
		...
		new JWTToken(token, wrongSecret,validIssuers);
	}
	...
	public void testVerifyToken_ThrowsInvalidTokenExceptionWhenTokenIsInvalid() throws Exception {
		...
		new JWTToken(token, broken,validIssuers);
	}
	...
	public void testVerifyToken_VerifiedWhenAlgorithmIsDifferent() throws Exception {
		...
		JWTToken jwtDec = new JWTToken(validIssuers);
	...
	public void testVerifyAuth0Token() throws Exception {
		...
		JWTToken auth0Jwt = new JWTToken(validIssuers) {
			@Override
    ...
    public void testParse() throws Exception {
		...
		String scope = "read:yyy";
		String gty = "client-credentials";
		String azp = "LU2Nt7YPHQ3lxrFNKitJ82syB4wIMR7G";
		@SuppressWarnings("serial")
		...
		assertEquals(scope, result.get(JWTToken.CLAIM_SCOPE));
		assertEquals(gty, result.get(JWTToken.CLAIM_GRANT_TYPE));
		assertEquals(azp, result.get(JWTToken.CLAIM_AUTHORIZED_PARTY));
