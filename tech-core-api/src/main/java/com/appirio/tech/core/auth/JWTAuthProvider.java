...
public class JWTAuthProvider <P extends AuthUser> extends AuthFilter<String, P> {
	...
	public static final String PATHPARAMNAME="authtoken";
    ...
    /**
     * This method will check the jwt token either in the header or path param.
     * The value in the header will take precedence.
	...
	protected String extractCredentials(ContainerRequestContext requestContext) {
		...
		if (header == null || header.trim().length() == 0) {
			// check the token in the path param
			String token = requestContext.getUriInfo().getQueryParameters().getFirst(PATHPARAMNAME);
			if (token != null) {
				return token;
			}
			return null;
